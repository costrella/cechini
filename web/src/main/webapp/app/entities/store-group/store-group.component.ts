...
export class StoreGroupComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.storeGroupService
      ...
      .subscribe(
        (res: HttpResponse<IStoreGroup[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IStoreGroup): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInStoreGroups(): void {
    this.eventSubscriber = this.eventManager.subscribe('storeGroupListModification', () => this.loadPage());
  }

  delete(storeGroup: IStoreGroup): void {
    const modalRef = this.modalService.open(StoreGroupDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IStoreGroup[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
